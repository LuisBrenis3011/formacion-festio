...
def test_comprobante_reserva_sin_pago_retorna_404(client):
...
#   Bloqueado por la MISMA causa raíz (TASK-BIZ-05): aprobar_pago_completo()
#   llama a notificar_confirmacion_reserva(..., db=db), y esa función no
#   tiene parametro "db" -> TypeError inmediato. Ademas requeriria construir
#   el flujo alterno completo (POST /reservas/eventos -> POST /reservas/iniciar,
#   que usa disponibilidad_service.py, no visto aun) solo para llegar al mismo
#   crash. No tiene sentido construir esa integracion compleja hasta que
#   TASK-BIZ-05 este resuelta.
#
# def test_comprobante_generado_despues_aprobacion(...)
#   Depende de que aprobar_pago_exitoso funcione. Tambien requeriria
#   app/domain/pagos/models.py para poder crear un Comprobante directamente
#   en la BD como atajo (sin pasar por el flujo completo).
# ---------------------------------------------------------------------------